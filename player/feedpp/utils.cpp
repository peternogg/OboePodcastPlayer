...
bool utils::try_fs_lock(const std::string& lock_file, pid_t & pid)
{
	...
	if (lockf(fd, F_TLOCK, 0) == 0) {
        std::string pidtext = utils::to_string<int>(getpid());
	...
	if (fd >= 0) {
		char buf[32];
        int len = static_cast<int>(read(fd, buf, sizeof(buf)-1));
		unsigned int upid = 0;
		buf[len] = '\0';
		sscanf(buf, "%u", &upid);
        pid = static_cast<pid_t>(upid);
		close(fd);
	}
	return false;
}
...
std::string utils::run_program(char * argv[], const std::string& input) {
	...
	default: {
		...
		char cbuf[1024];
        int rc2;
        while ((rc2 = static_cast<int>(read(opipe[0], cbuf, sizeof(cbuf)))) > 0) {
            buf.append(cbuf, static_cast<size_t>(rc2));
		}
		close(opipe[0]);
	}
	break;
...
std::string utils::strprintf(const char * format, ...)
{
    ...
    unsigned int len = static_cast<unsigned int>(vsnprintf(buffer, sizeof(buffer), format, ap)) + 1;
...
{
    const char * colors[] = { "black", "red", "green", "yellow", "blue", "magenta", "cyan", "white", "default", nullptr };
...
bool utils::is_valid_attribute(const std::string& attrib) {
    const char * attribs[] = { "standout", "underline", "reverse", "blink", "dim", "bold", "protect", "invis", "default", nullptr };
...
std::string utils::get_prop(xmlNode * node, const char * prop, const char * ns)
{
	...
	if (node)
	{
		xmlChar * value;
		if (ns)
            value = xmlGetProp(node, (const xmlChar *)prop);
		else
            value = xmlGetNsProp(node, (const xmlChar *)prop, (const xmlChar *)ns);
		if (value)
		{
...
std::wstring utils::clean_nonprintable_characters(std::wstring text)
{
	...
	{
        if (!iswprint(static_cast<wint_t>(text[idx])))
			text[idx] = L'\uFFFD';
	}
	return text;
}

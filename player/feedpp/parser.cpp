...
parser::parser(unsigned int timeout, const char * user_agent, const char * proxy, const char * proxy_auth, curl_proxytype proxy_type)
    : to(timeout), ua(user_agent), prx(proxy), prxauth(proxy_auth), prxtype(proxy_type), doc(nullptr), lm(0)
{
}
                       ...
                       const std::string& cookie_cache)
{
  ...
  curl_slist * custom_headers = nullptr;
  if (etag.length() > 0)
  {
...
{
    doc = xmlReadMemory(buffer, static_cast<int>(size), url, nullptr, XML_PARSE_RECOVER | XML_PARSE_NOERROR | XML_PARSE_NOWARNING);
    if (doc == nullptr)
	{
